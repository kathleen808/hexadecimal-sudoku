 ...
 * 
 * Examples 3 and 4 run slower because there are more empty cells. There are more options to choose from, and each time
 * a wrong legalVal is used, the method has to backtrack and try the next legalVal. Since the algorithm uses a for loop
 * to loop through all of the legalVals, it starts at the smallest legalVal. If the correct legalVal is a large value,
 * then the algorithm has to go through all of the previous legalVals, find that there is no solution for each of them,
 * backtrack, to finally reach the correct legalVal.
 */
